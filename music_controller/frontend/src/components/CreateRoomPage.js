...
function CreateRoomPage({
    initialVotesThreshold=2,
    update=false,
    roomCode=null,
    updateCallback=()=>{}
}) {

    const [votesThreshold, setvotesThreshold] = useState(initialVotesThreshold);
    const [succesMsg, setSuccessMsg] = useState("");
    const [errorMsg, setErrorMsg] = useState("");
    const navigate = useNavigate();

    const handleVotesChange = (event) => {
        setvotesThreshold(event.target.value)
    }
    ...
    return (
        <Grid container spacing={4}>
            ...
            <Grid item xs={12} align="center">
                <Typography component="h3" variant="h2"> {pageTitle} </Typography>
            </Grid>
            <Grid item xs={12} align="center">
                <FormControl>
                    <FormHelperText>
                        <div align="center">
                            Votes required to add a song to the queue
                        </div>
                    </FormHelperText>
                    <TextField 
                        required={true} 
